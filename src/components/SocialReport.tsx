...
import { Facebook, Music2, Eye, Heart, Users, RefreshCw, AlertCircle } from "lucide-react";
...
const PLATFORM_COLORS = { facebook: "#3b82f6", tiktok: "#10b981" };
...
export default function SocialReport({ selectedBrand, setSelectedBrand }: SocialReportProps) {
  const [activeSection, setActiveSection] = useState<"overview" | "facebook" | "tiktok">("overview");
  ...
  const followerTrendChart = useMemo(() => {
    const byDate = new Map<string, { date: string; facebook: number; tiktok: number }>();
    fbDailyForChart.forEach((r) => {
      const entry = byDate.get(r.date) || { date: r.date, facebook: 0, tiktok: 0 };
    ...
    ttByDate.forEach((val, date) => {
      const entry = byDate.get(date) || { date, facebook: 0, tiktok: 0 };
      entry.tiktok = val;
      byDate.set(date, entry);
    });
    return Array.from(byDate.values()).sort((a, b) => (a.date < b.date ? -1 : 1));
  }, [fbDailyForChart, tiktokDailyScoped]);

  const sortedTiktokPosts = useMemo(() => [...tiktokPostsScoped].sort((a, b) => (a.create_time < b.create_time ? 1 : -1)), [tiktokPostsScoped]);

  const sections: { id: "overview" | "facebook" | "tiktok"; label: string }[] = [
          ...
          {isLoading ? (
            ...
            <>
              <p className="text-[11px] text-slate-400">
                Chỉ so sánh những gì hai nền tảng cùng cung cấp được: follower hiện tại, số bài/video đã đăng, và tổng tương tác (like/comment/share hoặc
                reactions/comments/shares). Facebook không còn cấp reach/impressions cấp Page (Meta đã retire), TikTok public API không cấp profile views —
                nên "Tổng hợp" không có 2 chỉ số này; xem chi tiết từng nền tảng ở 2 tab riêng.
              </p>
              ...
              <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
                ...
                <PlatformStat icon={Users} label="Nội dung đã đăng" value={`${fmt(fbPostCount)} bài · ${fmt(sortedTiktokPosts.length)} video`} color="text-indigo-600 bg-indigo-50 border-indigo-200" />
              ...
              <div className="h-80 rounded-xl border border-slate-100 bg-white p-4 shadow-sm">
                <span className="block pb-4 text-xs font-bold uppercase tracking-wide text-slate-400">Follower theo ngày — Facebook vs TikTok</span>
